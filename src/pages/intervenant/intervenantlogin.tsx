...
const IntervenantLogIn: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (showSuccessToast) {
            timer = setTimeout(() => {
                ...
                navigate('/intervenant-home'); // Redirige après 5 secondes en cas de succès
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/login`, {  // Utilisation de la variable d'environnement
                method: 'POST',
                headers: {
            ...
            <div className="signin-right col">
                <form onSubmit={handleSubmit} style={{ width: "100%", paddingRight: "40px", fontWeight: "bold" }}>
                    ...
                    <p style={{ fontSize: "13px", textAlign: "center" }}>
                        Vous n'avez pas de compte ? <a href="/intervenant-signin" style={{textDecoration:"none",color:"orange"}}>S'inscrire</a>
                    </p>
                </form>
            </div>
        </div>
